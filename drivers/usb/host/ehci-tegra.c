...
static int tegra_ehci_resume(struct platform_device *pdev)
{
	...
	//dev_err(&pdev->dev, "ehci[%s] resume \n",&pdev->name );
	...
	{
	    regen2 =0; 
	    #if Foxconn_ENABLE_IRQ	
	    enable_irq_wake(tegra->irq);  	
	    #endif
	    dev_err(&pdev->dev, " call the [usb_phy_init] function\n");
	    usb_phy_init(get_usb_phy(tegra->phy));
	}
	//else
		//dev_err(&pdev->dev, "tegra_ehci_resume ret [%d]\n",ret);
