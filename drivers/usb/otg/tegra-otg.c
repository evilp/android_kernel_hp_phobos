				...
				enum usb_otg_state to)
{
	...
	// wolong
    /* if( (to==OTG_STATE_B_PERIPHERAL) && (host_enable==1) ) { 
        pr_info("%s(%d) force otg state from %s to %s\n", __func__, __LINE__, tegra_state_name(to), tegra_state_name(OTG_STATE_A_HOST) );
        to = OTG_STATE_A_HOST;
	} else if( (to==OTG_STATE_A_HOST) && (host_enable==2) ) {
        pr_info("%s(%d) force otg state from %s to %s\n", __func__, __LINE__, tegra_state_name(to), tegra_state_name(OTG_STATE_B_PERIPHERAL) );	    
