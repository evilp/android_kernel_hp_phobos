...
static int __init Get_close_5v_flag(char *str)
{
	int hw_version;
#if 1
	printk("[WOLO] close 5v str=[%s]\r\n",str);
	hw_version=ASC2BIN(str[0]);
	printk("[WOLO] hw_version=[%d]\r\n",hw_version);
	if(hw_version >2) g_is_close_hub_5v=1;
	else                     g_is_close_hub_5v=0;
	#else
	g_is_close_hub_5v=0;
#endif	
	return 1;
...
__setup("hw-version=", Get_close_5v_flag);
...
struct palmas *palmas_dev;  // export for wolo_diag
static void palmas_power_off(void)
{
	unsigned int  addr;
	int slave;				 
	pr_warn("+++++++palmas_power_off++++++++\n"); 			
	if (!palmas_dev)
		return;
#if 0		
	//by David add
	...
	regmap_write(palmas_dev->regmap[slave], addr, 0x00); 
	addr = PALMAS_BASE_TO_REG(PALMAS_PU_PD_OD_BASE,
			PALMAS_OD_OUTPUT_CTRL); 	
	regmap_write(palmas_dev->regmap[slave], addr, 0x80); 	
...
EXPORT_SYMBOL_GPL(Sleep_Red);
void Resume_White(void)
{
	unsigned int  addr,reg;
	int slave;		
	slave = PALMAS_BASE_TO_SLAVE(PALMAS_PU_PD_OD_BASE);
...
EXPORT_SYMBOL_GPL(Resume_White);
void palmas_suspend_regen2(void)
{
	unsigned int  addr,reg;
	int slave;				 
	if (!palmas_dev)
	...
	addr = PALMAS_BASE_TO_REG(PALMAS_RESOURCE_BASE,
...
EXPORT_SYMBOL_GPL(palmas_suspend_regen2);
void set_hub_disable(int disable)
{
	if(disable==1){
		gpio_set_value(TEGRA_GPIO_PU4, 1);
	}else{
		gpio_set_value(TEGRA_GPIO_PU4, 0);
	}			
}	
EXPORT_SYMBOL_GPL(set_hub_disable);
...
//by david add
	/*add LED control on Power on */
#if 0	
	slave = PALMAS_BASE_TO_SLAVE(PALMAS_LED_BASE);
	addr = PALMAS_BASE_TO_REG(PALMAS_LED_BASE, PALMAS_LED_PERIOD_CTRL);
	regmap_write(palmas->regmap[slave], addr, 0x30);
	addr = PALMAS_BASE_TO_REG(PALMAS_LED_BASE, PALMAS_LED_CTRL);
	regmap_write(palmas->regmap[slave], addr, 0x20);
	//addr = PALMAS_BASE_TO_REG(PALMAS_LED_BASE, PALMAS_PWM_CTRL1);
	//regmap_write(palmas->regmap[slave], addr, 0x03);	
	//addr = PALMAS_BASE_TO_REG(PALMAS_LED_BASE, PALMAS_PWM_CTRL2);
	//regmap_write(palmas->regmap[slave], addr, 0x80);	
#endif		

	gpio_request(TEGRA_GPIO_PU4, "hub_rest");
	gpio_set_value(TEGRA_GPIO_PU4, 0);
...
//by David add
	...
	addr = PALMAS_BASE_TO_REG(PALMAS_PU_PD_OD_BASE,
			PALMAS_PRIMARY_SECONDARY_PAD1); 
	regmap_write(palmas_dev->regmap[slave], addr, PALMAS_PRIMARY_SECONDARY_PAD1_LED2);			
	slave = PALMAS_BASE_TO_SLAVE(PALMAS_LED_BASE);
	addr = PALMAS_BASE_TO_REG(PALMAS_LED_BASE, PALMAS_LED_PERIOD_CTRL);
	regmap_write(palmas_dev->regmap[slave], addr, 0x28);
	addr = PALMAS_BASE_TO_REG(PALMAS_LED_BASE, PALMAS_LED_CTRL);
...
//control regen2
	...
	regmap_write(palmas_dev->regmap[slave], addr, reg);
#endif	
	pr_warn("--------------palmas_suspend--------\n");		
#endif
	return 0;
}

static int palmas_suspend_late(struct device *dev)
{
	//pr_warn("--------------palmas_suspend_late--------\n");	
//#if  Enable_CLOSE_USB_5V	
       if(g_is_close_hub_5v ==1)
       	{
       	        printk("[WOLO]Close 5V start!!!\r\n");
		palmas_suspend_regen2();
		//msleep(3600);	//for TP init
		mdelay(600);
		printk("[WOLO]Close 5V end!!!\r\n");
...
//by David add 
	...
	addr = PALMAS_BASE_TO_REG(PALMAS_PU_PD_OD_BASE,
			PALMAS_PRIMARY_SECONDARY_PAD1); 
	regmap_write(palmas_dev->regmap[slave], addr, PALMAS_PRIMARY_SECONDARY_PAD1_PWM2);			
	slave = PALMAS_BASE_TO_SLAVE(PALMAS_LED_BASE);
	addr = PALMAS_BASE_TO_REG(PALMAS_LED_BASE, PALMAS_LED_PERIOD_CTRL);
	regmap_write(palmas_dev->regmap[slave], addr, 0x18);
	addr = PALMAS_BASE_TO_REG(PALMAS_LED_BASE, PALMAS_LED_CTRL);
...
static const struct dev_pm_ops palmas_pm_ops = {
	SET_SYSTEM_SLEEP_LATE_PM_OPS(palmas_suspend, palmas_resume, palmas_suspend_late)
};
