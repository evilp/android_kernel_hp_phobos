...
//wolo_vh:get mac 
static bool g_asd_mode = false;

#define ASC2BIN(a)	((a >= '0' && a <= '9') ? (a - '0') : (a - 'A' + 10))

int a2bin(unsigned int byte1, unsigned int byte2)
{
	int	value;
	...
	if(byte1 == 0)
		...
		{
		value = ASC2BIN(byte1)*16;
		value +=ASC2BIN(byte2);
		}
	return (value);
}

static int __init smsc95xx_get_mac(char *str)
{
	int i=0,index=0;
	...
	for (i=0;i<6;i++)
	{
	    mac_addr[i]=a2bin(str[index],str[index+1]);
	    index+=2;	
	    printk(":%x",mac_addr[i]);	
	}
	//tmp[i] ='\0';
...
__setup("eth-mac=", smsc95xx_get_mac);
...
static void smsc95xx_init_mac_address(struct usbnet *dev)
{
		...
		netif_dbg(dev, ifup, dev->net,
				"MAC address read from ASD");
		//printk("[WOLO]MAC address read from ASD!!!!!! \r\n");
		return;        
        }
...
#if 0				
			smsc95xx_write_reg(dev, HCFG_PDS, 0x3E);		
			smsc95xx_write_reg(dev, HCFG_PDB, 0x3E);		
#endif				
      return 0;
 }
