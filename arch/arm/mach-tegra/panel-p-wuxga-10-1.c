...
#if foxconn_DSItoLVDS_Display
#define lcd_bl_en		TEGRA_GPIO_PH2
...
//b+
static void  dalmore_camera_power_on_for_mic(struct device *dev)
{
...
static int dalmore_dsi_regulator_get(struct device *dev)
{
	...
	pr_warn("dalmore_dsi_regulator_gett++++++++\n");
#endif	 
	if (reg_requested)
		return 0;
	...
	reg_requested = true;
#if foxconn_Display_Debug	
	pr_warn("dalmore_dsi_regulator_get----------\n");
#endif	 	
	return 0;
fail:
	return err;
...
static int dalmore_dsi_gpio_get(void)
{
...
#if foxconn_Display_Debug	
	pr_warn("dalmore_dsi_gpio_get++++++++\n");
#endif	 
	if (gpio_requested)
		return 0;
#if foxconn_DSItoLVDS_Display	
#if 0
	err = gpio_request(lcd_bl_en, "backlight enable");
	if (err < 0) {
	...
	err = gpio_request(lvds_en, "lvds enable");
	if (err < 0) {
		pr_err("panel reset gpio request failed\n");
		goto fail;
	}
...
#if foxconn_Display_Debug	
	pr_warn("macallan_dsi_regulator_get++++++++\n");
#endif	 
	if (reg_requested)
		return 0;
	avdd_lcd_3v3 = regulator_get(dev, "avdd_lcd");
...
//b+
	dalmore_camera_power_on_for_mic(dev); 
